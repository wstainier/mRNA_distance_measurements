#This code will perform the analysis of the points which are categorized as nascent peptides (i.e. translation points). It will output a distance measurement of those points to the surface 
#of the granule.
...
#Find the sub-pixel location of C3:Scfv-GFP (from FISH-Quant)
#Find the closest 3D surface point to each translation point and plot distances
...
arr3 = getPoints(userinput + '/FQ3DCoordinatesC3.csv')#translation points (scFv points)
...
#STEP 2 - Find the outline point and its index that each translation total point is closest to. Also find the granule point and its index that each translation total point is closest to
...
#STEP 3 - Find and plot distance between closest 3D surface point and translation total points & categorize whether point is inside or outside granule
mindist_dense = list()
...
np.savetxt(userinput + '/Translation_3DSurfaceDENSE_Distance.csv', mindist_dense ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/Translation_3DSurface_Distance.csv', mindist ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/Translation_Granule_Distance.csv', mindist_gra ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/Translation_ErodeSubtract_Distance.csv', mindist_erode ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/Translation_Background_Distance.csv', mindist_back ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/Translation_3DSurfaceALL_Distance.csv', mindist_3D_All ,delimiter=",", fmt='%s')
print('3D Surface ALL Granule CSV saved')

##Finding the translation points that are closest to 3D surfaces which are in both datasets

df_trans = pd.read_csv(userinput + '/FQ3DCoordinatesC3.csv', header = None, names = ['X', 'Y', 'Z'])

df_all = pd.read_csv(userinput + '/Translation_3DSurfaceALL_Distance.csv', header = None, names = ['All_3D_Surface'])

df_3D = pd.read_csv(userinput + '/Translation_3DSurface_Distance.csv', header = None, names = ['3D_Surface'])

df_3D_DENSE = pd.read_csv(userinput + '/Translation_3DSurfaceDENSE_Distance.csv', header = None, names = ['DENSE_3D_Surface'])
...
#getting the location of translation points whose closest DENSE 3D Surface is in both the "edge included" and "edge excluded"
#datasets – also concatenating the distance from DENSE 3D Surface to translation point 
...
#Now use the .to_csv to change that back to csv to get the final data – this CSV stores the x,y,z coordinates of the translation points used 
#as well as the distance to the DENSE 3D surface (distance is 4th column)

final_trans_df.to_csv(userinput + '/3D_Surface_Consistent_and_TranslationDistance.csv', header = True, index = False)
print('The number of consistent translation points is '  + str(len(consistent_index)) + ' out of ' + str(len(df_trans)) + ' total points.')
...
df.to_csv(userinput + '/FullLocation_Translation_Outline_Distance_notConsistent.csv', index=False)
...
final_all_trans_df.to_csv(userinput + '/ALL_DENSE_3D_Surface_Consistent_and_TranslationDistance.csv', header = True, index = False)
