#This code will perform the analysis of the points which are categorized as not translating mRNA. It will output a distance measurement of those points to the surface 
#of the granule.
...
#Find the sub-pixel location of not translated mRNA (from FISH-Quant)
...
arr3 = getPoints(userinput + '/FQ3DCoordinatesmRNANotTranslate.csv')#not translated mRNA
...
#STEP 2 - Find the outline point and its index that each mRNA not translated point is closest to. Also find the granule point and its index that each mRNA not translated point is closest to
...
#STEP 3 - Find and plot distance between closest 3D surface point and mRNA not translated points & categorize whether point is inside or outside granule
mindist_dense = list()
...
np.savetxt(userinput + '/mRNANotTranslate_3DSurfaceDENSE_Distance.csv', mindist_dense ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/mRNANotTranslate_3DSurface_Distance.csv', mindist ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/mRNANotTranslate_Granule_Distance.csv', mindist_gra ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/mRNANotTranslate_ErodeSubtract_Distance.csv', mindist_erode ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/mRNANotTranslate_Background_Distance.csv', mindist_back ,delimiter=",", fmt='%s')
...
np.savetxt(userinput + '/mRNANotTranslate_3DSurfaceALL_Distance.csv', mindist_3D_All ,delimiter=",", fmt='%s')
print('3D Surface ALL Granule CSV saved')

##Finding the not translated mRNA points that are closest to 3D surfaces which are in both datasets

df_trans = pd.read_csv(userinput + '/FQ3DCoordinatesmRNANotTranslate.csv', header = None, names = ['X', 'Y', 'Z'])

df_all = pd.read_csv(userinput + '/mRNANotTranslate_3DSurfaceALL_Distance.csv', header = None, names = ['All_3D_Surface'])

df_3D = pd.read_csv(userinput + '/mRNANotTranslate_3DSurface_Distance.csv', header = None, names = ['3D_Surface'])

df_3D_DENSE = pd.read_csv(userinput + '/mRNANotTranslate_3DSurfaceDENSE_Distance.csv', header = None, names = ['DENSE_3D_Surface'])
...
#getting the location of mRNA not translated points whose closest DENSE 3D Surface is in both the "edge included" and "edge excluded"
#datasets – also concatenating the distance from DENSE 3D Surface to mRNA not translated point 
...
#Now use the .to_csv to change that back to csv to get the final data – this CSV stores the x,y,z coordinates of the mRNA not translated points used 
#as well as the distance to the DENSE 3D surface (distance is 4th column)

final_trans_df.to_csv(userinput + '/3D_Surface_Consistent_and_mRNANotTranslateDistance.csv', header = True, index = False)
print('The number of consistent mRNA not translated points is '  + str(len(consistent_index)) + ' out of ' + str(len(df_trans)) + ' total points.')
...
df.to_csv(userinput + '/FullLocation_mRNANotTranslate_Outline_Distance_notConsistent.csv', index=False)
...
final_all_trans_df.to_csv(userinput + '/ALL_DENSE_3D_Surface_Consistent_and_mRNANotTranslateDistance.csv', header = True, index = False)
